function App() {
  return (
    <>
      <body>
        <div className="flex p-4 mb-4 flex-row flex-1">
          <div>
            <h1 className="text-2xl bg-red-500">
              LOGO Ecommerce Website
            </h1>
          </div>
          <div clasName="flex flex-row">
            <div className="bg-red-400"><a href="">settings</a></div>
            <div className="bg-red-300"><a href="">login</a></div>
          </div>
        </div>
        
        
        <header>
          <nav className="p-8 bg-green-500 text-center">
            <div>
              <a className="text-white" href="">Navigation Bar</a>
            </div>
          </nav>
          <div className="m-4">
            <h1 className="text-center text-9xl">
              BIG PICUTRE
            </h1>
            <p>Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
              Dolor expedita, est exercitationem voluptatibus repellat voluptates 
              deserunt officia nam maxime a nostrum porro sequi, vitae numquam deleniti minus 
              architecto reprehenderit optio.Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
              Dolor expedita, est exercitationem voluptatibus repellat voluptates 
              deserunt officia nam maxime a nostrum porro sequi, vitae numquam deleniti minus 
              architecto reprehenderit optio.Lorem ipsum dolor sit amet consectetur, adipisicing elit. 
              Dolor expedita, est exercitationem voluptatibus repellat voluptates 
              deserunt officia nam maxime a nostrum porro sequi, vitae numquam deleniti minus 
              architecto reprehenderit optio.
              </p>
          </div>
        </header>

        <footer>

        </footer>
      </body>
      
    </>
    
  )
}

export default App
